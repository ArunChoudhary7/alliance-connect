...
import { User, Building2, GraduationCap, Sparkles, Link as LinkIcon, FileText, Loader2, CheckCircle, XCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { checkUsernameAvailable, updateProfile } from "@/lib/supabase";
...
const handleSubmit = async () => {
  ...
  const canProceed = () => {
    if (step === 1) {
      return formData.username.length >= 3 && usernameAvailable && formData.full_name.length >= 2;
    }
    return true;
  };
